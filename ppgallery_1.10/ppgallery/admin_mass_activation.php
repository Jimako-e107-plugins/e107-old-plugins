...
if (isset($_POST['save']))
{
	...
	{
		if (is_int($count/5)) {$after="</tr><tr>";}
		$type=explode(".",$_POST['source'][$i]);
		$imagetype=$type[count($type)-1];
		rename("images/".$_POST['source'][$i],"images/".USERID.time().$count.".".$imagetype);
		$file_name=USERID.time().$count.".".$imagetype;
		...
		if ((strtolower($imagetype)=="jpg")or(strtolower($imagetype)=="jpeg"))
		{
			...
			$sql->db_Select_gen("INSERT INTO ".MPREFIX."ppgallery_images
									...
									VALUES ('".$_POST['is_gallery']."',
											...
											'".time()."',
											'".$description."')");
			$text.="<td><img src='images/".$file_name."' style='max-width:100px;' /></td>".$after;
		}
		...
		elseif ($_POST['source'][$i]=="") {}
		unset($after);
		unset($file_name);
		unset($quelle);
		unset($ziel);
		unset($imagetype);unset($breite);unset($hoehe);
		unset($altesBild);unset($neuesBild);unset($neueBreite);unset($neueHoehe);
		$count++;
	}
	$text.="</tr></table>";
	$ns -> tablerender($title,$text);
	...
	$text="
		<form action='".e_SELF."' method='post'>
			...
			$text.="
				...
				<tr>
					<td class='fcaption' style='vertical-align:top;width:10%;'>".PPGLAN_27."</td>
					<td class='forumheader3'>
						<table style='width:100%;'>
							<tr>
	";
	...
	while ($file=readDir($verzeichnis))
	{
		...
		if ($file!="." && $file != ".." && !is_dir($file)) {
				...
				strstr($file, ".jpg"))
			{
				...
				$bild1=$bild1['source'];
				if ($file!=$bild1) {$text.="<td><input type='checkbox' name='source[]' value='$file'><img src='images/$file' style='max-width:100px;' /></td>".$after;unset($after);$count++;}
