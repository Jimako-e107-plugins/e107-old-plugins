...
if ($faq_obj->faq_topnum>0)
{
    ...
    {
        $faq_arg = "select faq_id,faq_question,faq_views,faq_parent from #faq left join #faq_info on faq_parent = faq_info_id where faq_approved>0 and find_in_set(faq_info_class,'" . USERCLASS_LIST . "') order by faq_datestamp desc limit 0," . $faq_obj->faq_topnum;
        if ($sql->db_Select_Gen($faq_arg))
        {
            $faq_cache .= "<div class='forumheader3' style='text-align:center;'><strong>" . FAQLAN_137 . " " . $faq_obj->faq_topnum . " " . FAQLAN_136 . "</strong><br /><br />";
            while ($faq_row = $sql->db_Fetch())
            {
                ...
                $faq_cache .= "<a href='" . e_PLUGIN . "faq/faq.php?0.cat." . $faq_parent . "." . $faq_id . "' title='" . $tp->toHTML($faq_question) . "'>" . $tp->toHTML($faq_quest) . "</a> <br />";
            }
            $faq_cache .= "</div>";
        }
        else
