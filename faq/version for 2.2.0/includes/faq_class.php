...
class faq
{
    ...
    function faq()
    {
        ...
        define("FAQLAN_FAQ", $FAQ_PREF['faq_title']);
        /*
 		// For debugging
        if ($this->faq_super)
        {
...
*/
		...
		{
        /* its not needed, prefs are already in separate row, but with wrong name. */
		}
    ...
    function save_prefs()
    {
        ...
        $sql->db_Update("core", "e107_value='$tmp' where e107_name='plugin_faq'", false);
        return ;
    }
    function load_prefs()
    {
        ...
        if($row = $sql->retrieve("core", "*", "e107_name='plugin_faq' LIMIT 1")) {				   
				   $FAQ_PREF = $eArrayStorage->ReadArray($row['e107_value']);
				}
				elseif($row = $sql->retrieve("core", "*", "e107_name='faq' LIMIT 1")) {	
				    //try to load old name
				    $FAQ_PREF = $eArrayStorage->ReadArray($row['e107_value']);
				    $tmp = $eArrayStorage->WriteArray($FAQ_PREF);
            $sql->db_Insert("core", "'plugin_faq', '$tmp' ");
            $sql->db_Select("core", "*", "e107_name='plugin_faq' ");
				    print_a($row);
				}
				else {
