...
$rows = $sql->db_Rows();
$result = mysql_query("select game_title from ".MPREFIX."arcade_games");
...
$sql->db_Select("arcade_games", "game_id,game_title,game_filename,game_category", "order by rand() limit 1", "no_where");
		 ...
		 ";
		####end of random game
#show recently added games in menu
$text .= "<center><b>Most Recent Additions</b></center><br>";
$result = mysql_query("select game_id,game_title,game_category,game_filename,date_added from e107_arcade_games order by date_added desc limit 0,6");
  $text .= "<table>";
  $col = 1;
  while ( $row = mysql_fetch_array($result) ) {
    $dateadded = date("m/d/y", $row['date_added']);
    $text .= "<td><a href='".e_PLUGIN."kroozearcade_menu/play.php?catid=" . $row["game_category"] . "&gameid=" . $row["game_id"] . "'><img src='".e_PLUGIN."kroozearcade_menu/games/" . $row["game_filename"] . ".gif' /></a><br>
    <a href='".e_PLUGIN."kroozearcade_menu/play.php?catid=" . $row["game_category"] . "&gameid=" . $row["game_id"] .
     "'>" . $row["game_title"] . "</a><br>
     Date Added: $dateadded<br><br></td></center>";
      if ($col == 2) {
        $text .= "</tr>";
        $col = 1;
      }else{
...
/*		
this is being moved to a new menu for stats
