...
if (isset($_POST['update_cat'])) {
        $message = ($sql->db_Update("aacgc_mp_cat", "cat_title='".$tp->toDB($_POST['cat_title'])."', cat_det='".$tp->toDB($_POST['cat_det'])."', cat_order='".$tp->toDB($_POST['cat_order'])."' WHERE cat_id='".$_POST['id']."' ")) ? "".AMP_24."" : "".AMP_25."";
}

if (isset($_POST['main_delete'])) {
        $delete_id = array_keys($_POST['main_delete']);
	$sql2 = new db;
    $sql2->db_Delete("aacgc_mp_cat", "cat_id='".$delete_id[0]."'");
...
if ($action == "") {
        ...
        <tr>
        <td style='width:0%' class='forumheader3'>".AMP_39."</td>
        <td style='width:25%' class='forumheader3'>".AMP_35."</td>
        <td style='width:75%' class='forumheader3'>".AMP_36."</td>
        <td style='width:0%' class='forumheader3'>".AMP_37."</td>
...
if ($action == "edit")
{
                $sql->db_Select("aacgc_mp_cat", "*", "cat_id = $id");
                $row = $sql->db_Fetch();

$sql2 = new db;
$sql2->db_Select("aacgc_mp_cat", "*");
$rows = $sql2->db_Rows();
for ($i=0; $i < $rows; $i++) {
...
$next = $n + 1;
        ...
        <tr>
        <td style='width:30%; text-align:right' class='forumheader3'>".AMP_35.":</td>
        <td style='width:70%' class='forumheader3'>
            ".$rs -> form_text("cat_title", 100, $row['cat_title'], 500)."
        </td>
        </tr>
        <tr>
        <td style='width:; text-align:right' class='forumheader3'>".AMP_36.":</td>
	...
	<tr>
        <td style='width:30%; text-align:right' class='forumheader3'>".AMP_37.":</td>
        <td style='width:70%' class='forumheader3'>
...
";
        ...
        ".$rs -> form_button("submit", "update_cat", "".AMP_44."")."
        </td>
        </tr>
        </table>
        ".$rs -> form_close()."
        </div>";
	      $ns -> tablerender("AACGC Meeting Planner (".AMP_43.")", $text);
	      require_once(e_ADMIN."footer.php");
}
?>
