...
ob_start();
if (!class_exists('phpthumb_functions') && (!file_exists(dirname(__FILE__).'/phpthumb.functions.php') || !include_once(dirname(__FILE__).'/phpthumb.functions.php'))) {
	ob_end_flush();
...
$PHPTHUMB_CONFIG['cache_prefix'] = 'thumb_cache_'.str_replace('www.', '', @$_SERVER['SERVER_NAME']);
...
if (strtoupper(substr(PHP_OS, 0, 3)) == 'WIN') {
	...
	$PHPTHUMB_CONFIG['imagemagick_path'] = 'D:/develop/tools/ImageMagick/convert.exe';
} else {
...
$PHPTHUMB_CONFIG['error_message_image_default'] = '../images/gallery_120.png';       // Set this to the name of a generic error image (e.g. '/images/error.png') that you want displayed in place of any error message that may occur. This setting is overridden by the 'err' parameter, which does the same thing.
...
$PHPTHUMB_CONFIG['nooffsitelink_watermark_src'] = '../images/gallery_120.png';                // webroot-relative image to overlay on hotlinked images
...
$PHPTHUMB_CONFIG['border_hexcolor']     = 'C0C0C0'; // Default border color - usual HTML-style hex color notation (overidden with 'bc' parameter)
...
$PHPTHUMB_CONFIG['high_security_password']   = 'sD3fs^Sas@';     // required if 'high_security_enabled' is true, must be at least 5 characters long
...
function phpThumbURL($ParameterString) {
    ...
    return 'sgal_thumb.php?'.$ParameterString.'+hash='.md5($ParameterString.@$PHPTHUMB_CONFIG['high_security_password']);
}

function phpJsThumbURL($ParameterString) {
	global $PHPTHUMB_CONFIG;
	return 'showpic.php?'.$ParameterString.'+hash='.md5($ParameterString.@$PHPTHUMB_CONFIG['high_security_password']);
}
///////////////////////////////////////////////////////////////////////////////
