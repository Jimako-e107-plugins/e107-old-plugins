...
/*
   ...
   +---------------------------------------------------------------+
*/
$cpage_cache_tag = 'nq_cpage_ratemenu';
...
$cpage_arg = "select *,format(rate_rating/rate_votes,1) as rating from #cpage_page left join #rate on cpage_id=rate_itemid and rate_table='cpage' where cpage_rating_flag=1 and cpage_menu_flag=1 and rate_rating>0 and find_in_set(cpage_class,'" . USERCLASS_LIST . "') order by rate_rating desc,rand()  limit 0," . $CPAGE_PREF['cpage_inmenu'];
$cpage_pagecount = $sql->db_Select_gen($cpage_arg);
$cpage_text = CPAGE_MENU_MV05 ;

if ($sql->db_Select_gen($cpage_arg, false)) {
    while ($cpage_row = $sql->db_Fetch()) {
        $rate = $cpage_row['rating'];
...
$ns->tablerender(CPAGE_MENU_MV05, $cpage_text, 'cpage_menu_rated'); // Render the menu
