...
if (e_QUERY) {
	...
	if ($sql->db_Select('cpage_page', 'cpage_id,cpage_link', "where cpage_id={$tmp[0]}", 'nowhere', false)) {
		extract($sql->db_Fetch());
		$newloc = SITEURL  .'cpage/'. $cpage_obj->make_url($cpage_link,$cpage_id,0);
...
class pageClass {
    ...
    /* bullet image */
    var $pageText;
    /* main text of selected page, not parsed */
    var $multipageFlag;
    /* flag - true if multiple page page, false if not */
    var $pageTitles;
    /* array containing page titles */
    var $pageID;
    /* id number of page to be displayed */
    var $pageSelected;
    /* selected page of multiple page page */
    var $pageToRender;
    /* parsed page to be sent to screen */
    var $debug;
    /* temp debug flag */
    var $title;
    /* title of page, it if has one (as defined in [newpage=title] tag */
