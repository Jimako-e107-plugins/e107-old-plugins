...
/*
   ...
   +---------------------------------------------------------------+
*/
$cpage_cache_tag = 'nq_cpage_commentmenu';
...
$cpage_arg = "select * from (select *, count(comment_id) as totcomments from #cpage_page left join #comments on cpage_id=comment_item_id and comment_type='cpage'  where cpage_comment_flag=1   and cpage_menu_flag=1 and find_in_set(cpage_class,'" . USERCLASS_LIST . "')) as cpage_tmp where totcomments>0 order by totcomments,rand() desc limit 0," . $CPAGE_PREF['cpage_inmenu'];
// $cpage_pagecount = $sql->db_Select_gen($cpage_arg,false);
$cpage_text = CPAGE_MENU_MV06 ;

if ($sql->db_Select_gen($cpage_arg, false)) {
    while ($cpage_row = $sql->db_Fetch()) {
        $rate = $cpage_row['totcomments'];
        ...
        $cpage_text .= "<br /><br />
{$cpage_obj->bullet}
<span class='smalltext'><b><a href='" . SITEURL .  $cpage_obj->make_url($tp->toFORM($cpage_row['cpage_link']), $cpage_row['cpage_id'], 0,$tp->toFORM($cpage_row['cpage_title'])) . "'>" . $tp->toHTML($cpage_row['cpage_link'], false, "no_make_clickable emotes_off") . " ($rate)</a></b>
...
$ns->tablerender(CPAGE_MENU_MV06, $cpage_text, 'cpage_menu_commented'); // Render the menu
