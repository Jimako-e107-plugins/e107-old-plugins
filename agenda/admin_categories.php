...
//---------------------------------------------------------------
   ...
   if (isset($_POST['edit'])) {
      $agn_sql1->db_Select($agenda->getCategoryTable(), "*", "$primaryid='".$_POST['existing']."'", true, $agenda->isDebug());
   ...
   $table_total = $agn_sql1->db_Select($agenda->getCategoryTable(), "*", "order by cat_name asc", "", $agenda->isDebug());
   if (!$table_total) {
      $text .= LAN_EMPTY;
   } else {
