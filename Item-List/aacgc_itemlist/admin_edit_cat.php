...
if (isset($_POST['update_item_cat'])) {
        $message = ($sql->db_Update("aacgc_itemlist_cat", "item_cat_name='".$tp->toDB($_POST['item_cat_name'])."', item_cat_details='".$tp->toDB($_POST['item_cat_details'])."' WHERE item_cat_id='".intval($_POST['id'])."' ")) ? "Successful updated" : "Update failed";
}

if (isset($_POST['main_delete'])) {
        $delete_id = array_keys($_POST['main_delete']);
	$sql2 = new db;
    $sql2->db_Delete("aacgc_itemlist_cat", "item_cat_id='".intval($delete_id[0])."'");
...
if ($action == "") {
        ...
        <tr>
        <td style='width:0%'  class='forumheader3'>".LAN_AIL_CATEG_05."</td>
        <td style='width:40%' class='forumheader3'>".LAN_AIL_CATEG_02."</td>
        <td style='width:60%' class='forumheader3'>".LAN_AIL_CATEG_03."</td>
        ...
        $text .= $rs->form_close();
	      $ns -> tablerender(LAN_AIL_CATEG_08, $text);
...
if ($action == "edit")
{
        ...
        <tr>
        <td style='width:30%; text-align:right' class='forumheader3'>".LAN_AIL_CATEG_02.":</td>
        <td style='width:70%' class='forumheader3'>
            ".$rs -> form_text("item_cat_name", 100, $row['item_cat_name'], 500)."
        </td>
        </tr>
        <tr>
        <td style='width:30%; text-align:right' class='forumheader3'>".LAN_AIL_CATEG_03.":</td>
        <td style='width:70%' class='forumheader3'>
...
";
        ...
        ".$rs -> form_button("submit", "update_item_cat", LAN_AIL_CATEG_07)."
        </td>
        </tr>
        </table>
        ".$rs -> form_close()."
