...
if(strpos($_SERVER['HTTP_USER_AGENT'],"iPhone") !== false) {
	...
	header("location: iPhone.php") and exit;
...
if($auth === true AND $qry[0] == "del" AND $qry[1]) {
	...
	if($cat[0] == "new") {
		...
		header("location: tick.php".($qry[2] ? '?'.$qry[2] : '')."") and exit;
	...
	<table cellspacing='0' border='0' cellpadding='0'>
		...
		<tr>
			...
			<td valign='top' style='width:750px;'>
				...
				<h1>".$row[1]."</h1> <h3>".LAN_STARTED." ".$date."</h3>
		...
		<tr>
			...
			<td colspan='3' valign='top'>
				...
				<form method='post' action='".e_SELF."?new.".$qry[0]."'>
					...
					<textarea autocomplete='off' type='text' name='tick' class='tbox' rows='10' cols='250'></textarea>
					<input type='submit' name='submit' class='button' value='".LAN_SUBMIT."'>
		...
		print "
			...
			<tr {$del}>
				...
				<td valign='top' style='line-height:30px;'>
					...
					".($auth === true && $qry[0] == "adminm0de" ? " <a href='einzeln.php?".$tmp[0]."'># ".$tmp[0]."</a> <a title='".LAN_DEL."' href='".e_SELF."?del.".$tmp[0].".adminm0de'><img src='".e_IMAGE."admin_images/delete_16.png' border='0' /></a> <a title='".LAN_RESET."' href='".e_SELF."?reset.".$tmp[0].".adminm0de'><img src='".e_IMAGE."admin_images/plugins_16.png' border='0' /></a> <a title='".LAN_EDIT."' href='".e_SELF."?edit.".$tmp[0].".adminm0de'><img src='".e_IMAGE."admin_images/edit_16.png' border='0' /></a> " : "")."(<a {$style} href='".e_BASE."user.php?id.".$tmp[3]."'>".$tmp[4]."</a> @ ".$time."):<br>
...
if($_SERVER['REQUEST_METHOD'] == "POST" AND $qry[0] == "new" AND $auth === true) {
	...
	header("location: tick.php".($qry[1] ? '?'.$qry[1] : '')."") and exit;
