...
function DelGame(id) {
	...
	if(sure){
		http.open("GET","admin_old.php?DelGame&gid=" + id+"&code="+Math.floor(Math.random()*99999),true);
...
if(games > 1){
	...
	clanm_jq('#gamestable').tableDnD({
			onDrop: function(table, row) {
				...
				http.open("GET","admin_old.php?SaveGameOrder&neworder="+list+"&code="+Math.floor(Math.random()*99999),true);
				http.send(null);  
			}
		});
	});
