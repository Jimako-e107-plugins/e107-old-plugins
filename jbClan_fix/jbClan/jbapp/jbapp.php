...
if(USER) {
    ...
    // check if they are member or not otherwise it's confusing
		if (e107::isInstalled('jbroster_menu'))  {
			 $installed_jbroster = 1;
		}	
    else {
      if(getperms('0')) {
        $mes->addError('For Main Admin only: JBRoster plugin is not installed! You will get email. ');        
      }
    }  
    if ($installed_jbroster) { 
       $sql->select("user", "*", "user_id = ".intval(USERID));
       while ($row = $sql->db_Fetch()) {
        		$userEmail = $row['user_email'];
      }
      if ($sql->count(DB_TABLE_ROSTER_MEMBERS, "(*)", "WHERE member_id = ".intval(USERID)) > 0) {
    			// Don't insert this record
           $mes->addInfo('You are already roster member! Change your profile.');
           $roster_member = 1;             
      }
    }
    ...
    if($roster_member) {
      $heading = "You are already member of ".$organization_name;
    }
    else {
      $heading = LAN_JBAPP_MAIN_PAGE_I_WANT_TO_JOIN.$organization_name;
    }
		$text .= "
        <center>
            <p>
                <b><font size='+1'>  $heading</font></b>
            </p>
        </center>
        ...
        <p>";
    if($roster_member) {
         $text =  "<table class='tborder' cellspacing='15'><tr><td class='tborder' valign='top'> 
          You can change your member profile  
         <a href='".e_PLUGIN."jbroster_menu/member_profile.php?member_id=".USERID."'>here</a></td></tr></table>";
    }
    else {           
        $text .= "
                ...
                while($row = $sql->db_Fetch()) {
                	...
                	if ($row['attribute_id'] == 3) {
	                	...
	                	if ($row['attribute_id'] == 34) {
	                		...
	                		$text .= "
		                    <td class='tborder'>";
		                    	if ($row['attribute_id'] == 15) {    //age
                         
		                    		$text .= "
						      <input class='tbox form-control' type='text' name='".$row['attribute_id'].".month' size='2' maxlength='2' style='width: 20%;     display: inline;'> /
									<input class='tbox form-control' type='text' name='".$row['attribute_id'].".day' size='2' maxlength='2' style='width: 20%;     display: inline;'> /
									<input class='tbox form-control' type='text' name='".$row['attribute_id'].".year' size='4' maxlength='4' style='width: 20%;  display: inline;'> (MM/DD/YYYY)";
		                    	} else if ($row['attribute_id'] == 16) {    //gender
                            $options = array('male' => LAN_JBAPP_MAIN_PAGE_MALE, 'female' => LAN_JBAPP_MAIN_PAGE_FEMALE);
                            $text .= $frm->select($row['attribute_id'], $options);
		                    	...
		                    	} else if ($row['attribute_id'] == 32) {   //Skill Level
		                    ...
		                    /*		$text .= "
		                    	...
		                    	} else if ($row['attribute_id'] == 47) {  //memberinfo
                            $text .= $frm->textarea($row['attribute_id'], '', 4, 23, array('size'=>'xxlarge'));
		                    	//	$text .= "
						            //<textarea class='tbox' name='".$row['attribute_id']."' cols='23' rows='4'></textarea>";
		                    	} else if ($row['attribute_id'] == 48) {   // Application Comments
                          $text .= $frm->textarea($row['attribute_id'], '', 4, 23, array('size'=>'xxlarge'));
		                    	//	$text .= "
						            //<textarea class='tbox' name='".$row['attribute_id']."' cols='23' rows='4'></textarea>";
		                    	} else { 
                           
                            $text .=  $frm->text($row['attribute_id'], '', 25, 'size=large');
		                    		//$text .= "
