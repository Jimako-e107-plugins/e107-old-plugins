...
+---------------------------------------------------------------+
*/
$lan_file = e_PLUGIN."my_little_shop/languages/".e_LANGUAGE."/handler_lan.php";
require_once(file_exists($lan_file) ? $lan_file :  e_PLUGIN."my_little_shop/languages/German/handler_lan.php");
...
// --------------------------------- Dropdown menu 2 --------------------------------
	...
	case "dropdown2":
							...
							$ret ="<select class='tbox' style='width:150px'  name='".$fieldname."'><option></option>";
                            for ($i=0; $i<count($tmp); $i++) {
                            $split = explode(":",$tmp[$i]);
...
// --------------------------------- Color Selector --------------------------------
	case "color":
                            ...
                            $ret = Color_Selecte($fieldname,$presetvalue);
...
// --------------------------------- Date --------------------------------
	case "date":
    	                $tabmonth[1]= MLS_FORM_LAN_10;//"January";
	                    $tabmonth[2]= MLS_FORM_LAN_11;//"February";
	                    $tabmonth[3]= MLS_FORM_LAN_12;//"March";
	                    $tabmonth[4]= MLS_FORM_LAN_13;//"April";
	                    $tabmonth[5]= MLS_FORM_LAN_14;//"May";
	                    $tabmonth[6]= MLS_FORM_LAN_15;//"June";
	                    $tabmonth[7]= MLS_FORM_LAN_16;//"July";
	                    $tabmonth[8]= MLS_FORM_LAN_17;//"August";
	                    $tabmonth[9]= MLS_FORM_LAN_18;//"September";
	                    $tabmonth[10]= MLS_FORM_LAN_19;//"October";
	                    $tabmonth[11]= MLS_FORM_LAN_20;//"November";
	                    $tabmonth[12]= MLS_FORM_LAN_20;//"December";
...
// --------------------------------- Datestamp Unix --------------------------------
	...
	case "datestamp":
	                    ...
	                    $tabmonth[10]= MLS_FORM_LAN_19;//"October";
	                    $tabmonth[11]= MLS_FORM_LAN_20;//"November";
	                    $tabmonth[12]= MLS_FORM_LAN_21;//"December";
...
elseif($type =='short') {
    	                $tabmonth[1]= MLS_FORM_LAN_22;//"Jan";
	                    $tabmonth[2]= MLS_FORM_LAN_23;//"Feb";
	                    $tabmonth[3]= MLS_FORM_LAN_24;//"Mar";
	                    $tabmonth[4]= MLS_FORM_LAN_25;//"Apr";
	                    $tabmonth[5]= MLS_FORM_LAN_26;//"May";
	                    $tabmonth[6]= MLS_FORM_LAN_27;//"Jun";
	                    $tabmonth[7]= MLS_FORM_LAN_28;//"Jul";
	                    $tabmonth[8]= MLS_FORM_LAN_29;//"Aug";
	                    $tabmonth[9]= MLS_FORM_LAN_30;//"Sep";
	                    $tabmonth[10]= MLS_FORM_LAN_31;//"Oct";
	                    $tabmonth[11]= MLS_FORM_LAN_32;//"Nov";
	                    $tabmonth[12]= MLS_FORM_LAN_33;//"Dec";
}
...
function Color_Selecte($field,$dbvalue){
 ...
 /* $text .="<table cellspacing=\"0px\" cellpadding=\"4\" style='width:100%;border:0px'>
     <tr>
      ...
      <td style='width:100%'>&nbsp;<input class='button' type='button' value='".LAN_ADM_MYS_37a."' onClick='expandit(this)'>
  ...
  $text .= "&nbsp;#<input class='tbox' type=\"text\" name=\"".$field."\" id=\"".$field."\" value=\"".$dbvalue."\" size='15' onblur=\"View('".$field."',this.value)\" />
      &nbsp;<input class='button' type='button' value='".MLS_FORM_LAN_7."' onclick='expandit(this)' />";
...
//////+++++++++++++++++++++++++++
function powered_by_shop()
{
$linkinhalt="<div class='smalltext' style='width:100%; text-align: center;'>:: Powered by My Little Shop V.".MLS_VERSION." - from <a target='_blank' href='http://www.e107.4xa.de' title='besuche mich'>e107.4xa.de</a>::</div>";
return $linkinhalt;
}
//////+++++++++++++++++++++++++++
function to_prise($prise_in, $steuer)
{
 $St=($prise_in/100)*$steuer;
 $A=sprintf("%10.2f", ($prise_in+$St));
 return $A;
}
//////+++++++++++++++++++++++++++
...
///////////////////////////////
function image_upload($file,$folder)
{
$filedata = getimagesize($file);
$filetypes=check_image_typ($filedata);
$groesse=check_image_size($filedata);
echo $filetypes;
echo "-";
echo $filedata[2];
if ($filetypes=="0")
	{
	$Ausgabe['text']= "falsche Dateiformat!";
	$Ausgabe['wert']=0;
	return $Ausgabe;	
	}
else
	...
	if ($groesse!=1)
		...
		$file_name="";
		$file_name.=time();
		$file_name.="_".USERNAME;
		if(!copy ($file,$folder."".$file_name.".".$filetypes."")){
			$Ausgabe['text']= "Ups, es passierte ein Fehler beim Kopieren";
			$Ausgabe['wert']=0;
			return $Ausgabe;
			} else {
			$Ausgabe['text']= "Die Datei (".$file.") wurde erfolgreich auf den Server gelagen!";
			$Ausgabe['wert']=$file_name.".".$filetypes;
			return $Ausgabe;
			}
		}
	}
...
//////////////////////
function check_image_typ($filedata)
{
if($filedata[2]==1){return "gif";}
elseif($filedata[2]==2){return "jpg";}
elseif($filedata[2]==3){return "png";}
else{return "0";}
//return 0;
}
//////////////////////
function check_image_size($filedata)
{
$Max_Groesse['x']= PRODUCT_IMG_MAX_X;	
$Max_Groesse['y']= PRODUCT_IMG_MAX_Y;
if($filedata[0]< 1)
	{return LAN_IGM_NO_DATA;}
elseif($filedata[0]> 0 && $filedata[0] > $Max_Groesse['x'])
	{return "mit ".$filedata[0]."px, ".LAN_IGM_ZU_BREIT."".$Max_Groesse['x']."px!";}
elseif($filedata[1]> 0 && $filedata[1] > $Max_Groesse['y'])
	{return "mit ".$filedata[1]."px, ".LAN_IGM_ZU_HOCH."".$Max_Groesse['y']."px!";}
else{return 1;}
}
//////////////////////
function get_prew_image($image,$link,$name,$size,$link_color)
{if(!$link_color){$color="";}else{$color="color:#".$link_color.";";}
if(is_file($image))
	{
	$Tool_Desc=$name;
	$Tool_Tip_Text="<div border=\'0\' style=\'text-align:right;background:#fff;\'><img src=\'".$image."\' width=\'".$size."\' /><br\/>".$Tool_Desc."<\/div>";		
	return "<a href=\"".$link."\" style=\"".$color."\" onmouseover=\"Tip('$Tool_Tip_Text', TITLE, 'Vorschau', WIDTH, ".$size.", SHADOW, false, STICKY, 1, CLOSEBTN, true, CLICKCLOSE, true, TITLEBGCOLOR, '#000', TITLEFONTCOLOR, '#fff', BGCOLOR, '#fff', BORDERCOLOR, '#000')\" onmouseout=\"UnTip()\" ><b>".$name."</b></a>";
	}
else{
	return   "<font style='".$color."'>$name</font>";
	}
}
//////////////////////
function get_prew_image3($image,$link,$name,$size,$link_color)
{if(!$link_color){$color="";}else{$color="color:#".$link_color.";";}
if(is_file($image))
	{
	$Tool_Desc=$name;
	$Tool_Tip_Text="<div border=\'0\' style=\'text-align:right;background:#fff;\'><img src=\'".$image."\' width=\'".$size."\' /><br\/>".$Tool_Desc."<\/div>";		
	return "<a href=\"".$link."\" style=\"".$color."\" onmousklick=\"Tip('$Tool_Tip_Text', TITLE, 'Vorschau', WIDTH, ".$size.", SHADOW, false, STICKY, 1, CLOSEBTN, true, CLICKCLOSE, true, TITLEBGCOLOR, '#000', TITLEFONTCOLOR, '#fff', BGCOLOR, '#fff', BORDERCOLOR, '#000')\" onmouseout=\"UnTip()\" ><b>".$name."</b></a>";
	}
else{
	return   "<font style='".$color."'>$name</font>";
	}
...
//////////////////////////////
function get_prew_image2($image,$image2,$link,$name,$size,$size2,$default)
{
if(is_file($image))
	{
	$Tool_Desc=$name;
	$Tool_Tip_Text="<div border=\'0\' style=\'text-align:right;background:#fff;\'><img src=\'".$image."\' width=\'".$size."\' /><br\/>".$Tool_Desc."<\/div>";		
	return "<a href=\"".$link."\" onmouseover=\"Tip('$Tool_Tip_Text', TITLE, 'Vorschau', WIDTH, ".$size.", SHADOW, false, STICKY, 1, CLOSEBTN, true, CLICKCLOSE, true, TITLEBGCOLOR, '#000', TITLEFONTCOLOR, '#fff', BGCOLOR, '#fff', BORDERCOLOR, '#000')\" onmouseout=\"UnTip()\" ><img src=\"".$image2."\" width=\"".$size2."\" /></a>";
	//return "<img src='".$image."' width='".$size2."'/>";
	}
else{
	return "<img src='".$default."' width='".$size2."'/>";
	}
}
