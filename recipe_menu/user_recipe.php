...
if (!$pref['recipe_menu_adminclass'])
...
if ($recipemenu_action == 'update')
{
    ...
    $recipemenu_tmp = explode(".", $_POST['recipe_user']);
    $recipemenu_userid = $recipemenu_tmp[0];
	$recipemenu_username = substr($_POST['recipe_user'], strpos($_POST['recipe_user'], ".")+1);
    ...
    // end picture upload
    if (check_class($pref['recipe_menu_autoclass']))
    {
        $_POST['recipe_approved'] = 1;
    }
    if ($recipemenu_recipeid == 0)
    {
		...
		'" . $tp->toDB($_POST['recipemenu_select']) . "','" . $_POST['recipe_approved'] . "'," . time() . ",'" . $file . "',0,''" ;
...
if (!$recipemenu_edit)
{
    ...
    if ($sql2->db_Select("recipemenu_recipes", "recipe_id,recipe_name", "where  SUBSTRING_INDEX(recipe_author,'.',1)= '" . USERID . "' order by recipe_name", "nowhere", false))
    {
	...
	<tr><td colspan='2' class='forumheader3'><a href='recipes.php?0.show.0.0.0'><img src='./images/updir.png' style='border:0;' alt='" . RCPEMENU_15 . "' title='" . RCPEMENU_15 . "' /></a></td></tr>
	$recipemenu_msg
