...
class myf_form{
	...
	function user_extended_element_edit($form_ext_name,$presetvalue, $fieldname){
                        ...
                        global $pref,$key,$sql,$tp,$user_pref;$_POST;
                        $ut = explode("|",$form_ext_name);
                        $u_name = ($ut[0] != "") ? $ut[0] : trim($form_ext_name);
                        $u_type = trim($ut[1]);
                        $u_value = stripslashes($ut[2]);
                        $field_para=explode("~",$u_value);
                        $field_pflicht=$field_para[1];
                        $tmp = explode(";",$field_para[0]);
												$ret="";
                        switch ($u_type) {
...
// --------------------------------- Radio buttons --------------------------------
	case "radio":
                            ...
                            list($wert, $fdftext) = explode(":",$tmp[$i]);
                            $checked = ($tmp[$i] == $presetvalue)? " checked='checked'" : "";
                            $ret .="<input  type='radio' name='".$fieldname."'  value='".$wert."' $checked  /> $fdftext <br />";
...
// --------------------------------- Textbox --------------------------------
	case "text":
                            ...
                            $size = ($tmp[1]) ? $tmp[1]:40;
                            $max_size= ($tmp[2]) ? $tmp[2]:200;
                            $onclick= ($tmp[3]) ? "onclick=\"this.value=''\"":"";
                            $ret .="<input class='tbox' type='text' name='".$fieldname."' size='$size' value='".$tp->toDB($valuehere) ."' maxlength='$max_size' ".$onclick."/>";

                            break;
// --------------------------------- Textbereich --------------------------------
	case "text2":							
														$valuehere = ($presetvalue !="")? $presetvalue : $tmp[0];
                            $ret .="<textarea name='".$fieldname."' cols='60' rows='8'>".$tp->toDB($valuehere)."</textarea>";

                            break;
// --------------------------------- Color Selector --------------------------------
	case "color":

                            $ret = Color_Select33($fieldname,$presetvalue);
...
// --------------------------------- Table --------------------------------
	case "table":
                            ...
                            $sql -> db_Select($tmp[0],"*","$tmp[2] !='' ORDER BY $tmp[3]");
...
// --------------------------------- upload --------------------------------
	...
	case "upload":
														$ret = 	"<input type='file' name='".$fieldname."'>";
                          break;
// --------------------------------- Directory --------------------------------
    case "dir":
...
// --------------------------------- Images --------------------------------
	case "image":
        ...
        <input class='button' type ='button' style='cursor:pointer' size='30' value='".LAN_4xA_FORM_37."' onclick='expandit(this)' />
        <div id='".$fieldname."_box' style='display:none'>";

        foreach($iconlist as $key=>$icon){
        $ret .= "<a href=\"javascript:insertext('$icon','$fieldname','".$fieldname."_box')\" ><img src='".$folder.$icon."' style='border:0px;width:50px;' alt=''/></a> ";
        }
...
// --------------------------------- Access table --------------------------------
	...
	case "accesstable":
                            ...
                            $ret .="<option value='0' $checked >".LAN_4xA_FORM_38."</option>";
                            $checked = ($presetvalue == 252)? " selected='selected'" : "";
                            $ret .="<option value='252' $checked >".LAN_4xA_FORM_39."</option>";
                            $checked = ($presetvalue == 253)? " selected='selected'" : "";
                            $ret .="<option value='253' $checked >".LAN_4xA_FORM_40."</option>";
                            $checked = ($presetvalue == 254)? " selected='selected'" : "";
                            $ret .="<option value='254' $checked >".LAN_4xA_FORM_41."</option>";
                            $checked = ($presetvalue == 255)? " selected='selected'" : "";
                            $ret .="<option value='255' $checked >".LAN_4xA_FORM_42."</option>";
...
// --------------------------------- kalender --------------------------------
	case "caleder":
                        		...
                        		$weekNumbers =($tmp[3]);
         									  ...
         									  $data_Value = ($_POST[$fieldname] > 0) ? date("d/m/Y", $_POST[$fieldname]) : "";  
...
// --------------------------------- Date --------------------------------
	case "date":
    	                $tabmonth[1]= LAN_4xA_FORM_43;//"January";
	                    $tabmonth[2]= LAN_4xA_FORM_44;//"February";
	                    $tabmonth[3]= LAN_4xA_FORM_45;//"March";
	                    $tabmonth[4]= LAN_4xA_FORM_46;//"April";
	                    $tabmonth[5]= LAN_4xA_FORM_47;//"May";
	                    $tabmonth[6]= LAN_4xA_FORM_48;//"June";
	                    $tabmonth[7]= LAN_4xA_FORM_49;//"July";
	                    $tabmonth[8]= LAN_4xA_FORM_50;//"August";
	                    $tabmonth[9]= LAN_4xA_FORM_51;//"September";
	                    $tabmonth[10]= LAN_4xA_FORM_52;//"October";
	                    $tabmonth[11]= LAN_4xA_FORM_53;//"November";
	                    $tabmonth[12]= LAN_4xA_FORM_54;//"December";
...
function viewdate($date0,$type='short'){  //format date0 2003-11-01
	...
	elseif($type =='short') {
    	                $tabmonth[1]= LAN_4xA_FORM_55;//"Jan";
	                    $tabmonth[2]= LAN_4xA_FORM_56;//"Feb";
	                    $tabmonth[3]= LAN_4xA_FORM_57;//"Mar";
	                    $tabmonth[4]= LAN_4xA_FORM_58;//"Apr";
	                    $tabmonth[5]= LAN_4xA_FORM_59;//"May";
	                    $tabmonth[6]= LAN_4xA_FORM_60;//"Jun";
	                    $tabmonth[7]= LAN_4xA_FORM_61;//"Jul";
	                    $tabmonth[8]= LAN_4xA_FORM_62;//"Aug";
	                    $tabmonth[9]= LAN_4xA_FORM_63;//"Sep";
	                    $tabmonth[10]= LAN_4xA_FORM_64;//"Oct";
	                    $tabmonth[11]= LAN_4xA_FORM_65;//"Nov";
	                    $tabmonth[12]= LAN_4xA_FORM_66;//"Dec";
		}
...
function Color_Select33($field,$dbvalue){
  $dbvalue = $dbvalue == "" ? "000000" : $dbvalue;
  $bgcolor = "#".$dbvalue;
  $fdftext= "";
 /* $fdftext .="<table cellspacing=\"0px\" cellpadding=\"4\" style='width:100%;border:0px'>
     <tr>
      ...
      <td style='width:100%'>&nbsp;<input class='button' type='button' value='".LAN_4xA_FORM_37."' onClick='expandit(this)'>
  ...
  $fdftext .= "<span style=\"background-color: $bgcolor; border:1px solid black; height: 25px\">";
  $fdftext .= "<span id=\"ColorPreview_".$field."\" style=\"border:1px;height: 100%; width: 28px\">&nbsp;&nbsp;&nbsp;&nbsp;</span></span>";
  $fdftext .= "&nbsp;#<input class='tbox' type=\"text\" name=\"".$field."\" id=\"".$field."\" value=\"".$dbvalue."\" size='15' onblur=\"View('".$field."',this.value)\" />
      &nbsp;<input class='button' type='button' value='".LAN_4xA_FORM_37."' onclick='expandit(this)' />";
...
////////////////////////////////////////////////////////////////////
///////////////////////////////
function file_upload($file,$folder,$filename,$file_types,$file_size)
{
	
if(!$file_size){
	$MAX_FILEGRUESSE=99999; ///in bytes 999999 ist ca 1 MB	
	}
else{
	$MAX_FILEGRUESSE=$file_size;
	}
$filetype_tmp = explode('.', $filename);
$TT_temp=count($filetype_tmp);
$my_filetyp= $filetype_tmp[$TT_temp-1];
$groesse  = filesize($file);
$groesse_text = datei_groesse($groesse);
$MAX_FILEGRUESSE_TEXT=datei_groesse($MAX_FILEGRUESSE);
if( $groesse > $MAX_FILEGRUESSE)
	{
	$Ausgabe['text']= LAN_4xA_FORM_131." <b>".$filename."</b> ".LAN_4xA_FORM_132." <font style='color:#ff0000;font-size:120%;'><b>*".$groesse_text."*</b></font> ".LAN_4xA_FORM_133." ".$MAX_FILEGRUESSE_TEXT;
	$Ausgabe['wert']=0;
	$Ausgabe['folder']=$folder;
	$Ausgabe['file']=$filename;
	return $Ausgabe;	
	}
if (is_readable(e_ADMIN."filetypes.php")) 
{
	if(!$file_types)
  	...
  	$a_filetypes =$file_types;
  	}
  $a_filetypes = explode(',', $a_filetypes);
  for($i=0; $i< count($a_filetypes); $i++)
 		{
 		$sa_filetypes[$i] = trim($a_filetypes[$i]);	
 		}
}
$flagg1= false;
for($i=0;$i < count($sa_filetypes) ; $i++)
	{
	if($sa_filetypes[$i]==$my_filetyp)
		{
			$flagg1=true;
			if(!copy ($file,$folder.$filename)){
					$Ausgabe['text']= LAN_4xA_FORM_127; ///Keine Datei wurde hoch geladen
					$Ausgabe['wert']=0;
					$Ausgabe['folder']=$folder;
					$Ausgabe['file']=$filename;
					return $Ausgabe;
			}else{
					$Ausgabe['text']= "".LAN_4xA_FORM_128." <b>".$filename."</b> (".$groesse_text.") ".LAN_4xA_FORM_129. LAN_4xA_FORM_167.$folder;
					$Ausgabe['wert']=1;
					$Ausgabe['folder']=$folder;
					$Ausgabe['file']=$filename;
					return $Ausgabe;
...
$Ausgabe['text']= "<font style='color:#ff0000;font-size:120%;'><b>*".$my_filetyp."*</b></font> ".LAN_4xA_FORM_130;
$Ausgabe['wert']=0;
$Ausgabe['folder']=$folder;
$Ausgabe['file']=$filename;	
return $Ausgabe;
...
//////////////////////
////////////////////////////////////////////////////
function datei_groesse($DAT_B)
{
if($DAT_B > 1000)
	{
	if($DAT_B > 1000000)
		{
		return "".(round(($DAT_B / 1000000),2))." Mb";
		}
	return "".(round(($DAT_B / 1000),2))." kb";
	}
return "".$DAT_B." b";
}
///////////////////////////////////////////////////////////////////////////////
function filetypes_chek($params)
{
 $tmp = explode(';', $params);	
$file_types=$tmp[1];	
	if(!$file_types)
  ...
  for($i=0; $i< count($a_filetypes); $i++)
 		{
 		$sa_filetypes.= "*.".trim($a_filetypes[$i]).", ";	
 		}	
return	$sa_filetypes;
}
///////////////////////////////////////////////////////////////////////////////
function filesize_chek($params)
{
 	$tmp = explode(';', $params);	
	$file_size=intval($tmp[2]);	
...
if(!$file_size){
	...
	return datei_groesse($MAX_FILEGRUESSE);
}
///////////////////////////////////////////////////////////////////////////////
function get_filefolder($params)
{
 	$tmp = explode(';', $params);	
	$folder=$tmp[0];	
	return $folder;
