...
if($conf['inactiveafter'] > 0){
	...
	$sql->db_Select_gen("SELECT i.userid FROM #clan_members_info i, #user u WHERE u.user_id=i.userid AND u.user_lastvisit < $expiredate AND u.user_currentvisit < $expiredate");
		while($row = $sql->db_Fetch()){
			$member = $row['userid'];
			$sql1->db_Update("clan_members_info", "active='0' WHERE userid='$member'");
		}
}
...
//, u.user_msnm, u.user_icq, u.user_yim, u.user_aim, u.points, u.user_posts
	...
	$sql1->db_Select_gen("SELECT l.userid, u.user_name, r.rank, r.rimage, i.birthday, i.joindate, i.avatar, i.xfire, i.steam, i.active, i.realname, i.gender, i.location, i.country 
		FROM #".$linktable." l
