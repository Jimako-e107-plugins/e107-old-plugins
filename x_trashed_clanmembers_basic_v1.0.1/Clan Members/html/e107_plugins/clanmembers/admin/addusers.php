...
for($i=0;$i<count($members);$i++){
	if($members[$i] != ""){
		...
		if($sql->db_Count("clan_members_info", "(*)", "userid='".$members[$i]."'") == 0){
			$sql->db_Insert("clan_members_info", array("userid" => $members[$i], "country"=> "Unknown"));
		}
		...
		for($j=0;$j<count($games);$j++){
			if($games[$j] != ""){
				if($sql->db_Count("clan_members_gamelink", "(*)", "userid='$members[$i]' and gid='$games[$j]'") == 0){
					$sql->db_Insert("clan_members_gamelink", array("id" => "NULL", "gid" => $games[$j], "userid"=>$members[$i]));
				}
			}
		}
		//Add teams
		for($j=0;$j<count($teams);$j++){
			if($teams[$j] != ""){
				if($sql->db_Count("clan_members_teamlink", "(*)", "userid='$members[$i]' and tid='$teams[$j]'") == 0){
					$sql->db_Insert("clan_members_teamlink", array("id" => "NULL", "tid" => $teams[$j], "userid"=>$members[$i]));
				}
			}
		}
	}
