...
* $Id: ecal_class.php 13095 2013-04-11 19:47:45Z e107steved $
*
*/
...
class ecal_class
{
	...
	var $time_now;						// Current time/date stamp ('clock time')
	...
	function cal_log($event_type, $event_title = '', $event_string='', $event_start=0, $eventId = 0)
	{
		...
		if ($event_start > 0)
		{
			...
			$cmessage .= 'Event Link:  '.$pref['siteurl'].$PLUGINS_DIRECTORY. "calendar_menu/event.php?".$event_start.'.event.'.$eventId.' <br />';
		}
		else
		...
		switch ($event_type)
		{
			case 5 :
			case 1 : 
				$e_event -> trigger("ecalnew", $edata_ec);
				break;
			case 2 :
			case 3 :
			case 4 : 
		...
		$logDetail = $log_titles[$event_type];
		if  ($event_start > 0) $logDetail .= ' '.strftime('%d-%B-%Y',$event_start);
