...
if($conf['enablemail'] && $conf['allowsubscr'] && cansubscribe()){
	...
	$totalwars = $sql->db_Count("clan_wars", "(*)", "WHERE active='1' AND (opp_tag!='' OR opp_name!='')");	
	if($totalwars==0){
	...
	$nrwars = $sql->db_Count("clan_wars", "(*)", "WHERE active='1' AND (opp_tag!='' OR opp_name!='') $wheresql");		
	$win = $sql->db_Count("clan_wars", "(*)", "WHERE active='1' AND (opp_tag!='' OR opp_name!='') AND our_score > opp_score AND status='1'");
	$lose = $sql->db_Count("clan_wars", "(*)", "WHERE active='1' AND (opp_tag!='' OR opp_name!='') AND  opp_score > our_score AND status='1'");
	$draw = $sql->db_Count("clan_wars", "(*)", "WHERE active='1' AND (opp_tag!='' OR opp_name!='') AND  opp_score = our_score AND status='1'");		
	$upcomming = $sql->db_Count("clan_wars", "(*)", "WHERE active='1' AND (opp_tag!='' OR opp_name!='') AND  status='0'");
...
$tables = "";
if($orderby==_WDESC && $sortby==_WDATE){$ordersort="w.wardate DESC";}
elseif($orderby==_WASC && $sortby==_WDATE){$ordersort="w.wardate ASC";}
elseif($orderby==_WDESC && $sortby==_WGAME){
	$ordersort="g.gname DESC, w.wardate DESC";
	$tables = ", #clan_games as g";
	$wheresql .= " AND w.game=g.gid";
}elseif($orderby==_WASC && $sortby==_WGAME){
	$ordersort="g.gname ASC, w.wardate DESC";
	$tables = ", #clan_games as g";
	$wheresql .= " AND w.game=g.gid";
}elseif($orderby==_WDESC && $sortby==_WTEAM){
	$ordersort="t.team_tag DESC, w.wardate DESC";
	$tables = ", #clan_teams as t";
	$wheresql .= " AND w.team=t.tid";
}elseif($orderby==_WASC && $sortby==_WTEAM){
	$ordersort="t.team_tag ASC, w.wardate DESC";
	$tables = ", #clan_teams as t";
	$wheresql .= " AND w.team=t.tid";
}elseif($orderby==_WDESC && $sortby==_WOPP){$ordersort="w.opp_tag DESC, w.wardate DESC";}
elseif($orderby==_WASC && $sortby==_WOPP){$ordersort="w.opp_tag ASC, w.wardate DESC";}
elseif($orderby==_WDESC && $sortby==_WSTYLE){$ordersort="w.style DESC, w.wardate DESC";}
elseif($orderby==_WASC && $sortby==_WSTYLE){$ordersort="w.style ASC, w.wardate DESC";}
elseif($orderby==_WDESC && $sortby==_WPLAYERS){$ordersort="w.players DESC, w.wardate DESC";}
elseif($orderby==_WASC && $sortby==_WPLAYERS){$ordersort="w.players ASC, w.wardate DESC";}
	...
	$upcshown = false;
	$finshown = false;
	$sql->db_Select_gen("SELECT w.* from #clan_wars w $tables WHERE w.active='1' AND (w.opp_tag!='' OR w.opp_name!='') $wheresql ORDER BY ".($conf['seperate']?"status ASC,":"")." $ordersort LIMIT $min , ".$conf['rowsperpage']);
		if(!$sql->db_Rows()){
			$text .= "<tr><td class='forumheader3' colspan='".(($teams) ? 7 : 6)."'>Der er ingen kampe at vise</td></tr>";
		}
		$sql1 = new db;
		while ($row = $sql->db_Fetch()) {
			$wid = $row['wid'];
			...
			if($conf['seperate'] && ($selstatus == _WALL || $selstatus == "")){
				if(!$upcshown && !$status){
					$text .= "<tr><td class='fcaption' colspan='".(($teams) ? 7 : 6)."'><b>Kommende kampe</b></td></tr>";
					$upcshown = true;
				}
				if(!$finshown && $upcshown && $status){
					$text .= "<tr><td class='fcaption' colspan='".(($teams) ? 7 : 6)."'><b>Afsluttede kampe</b></td></tr>";
					$finshown = true;
				}
			}
		...
		$text .= "<tr class='forumheader3 iconpointer' title='Vis kamp detaljer' onmouseover=\"TdHover($wid);\" onmouseout=\"TdOut($wid);\" onclick=\"window.location='clanwars.php?Details&wid=$wid'\" id='war$wid'>
