...
if($status==0){
	...
	$text .= "<tr ".(($available>0) ? "" : "style='display:none;'")." id='travailable'>
			...
			<td align='left' class='forumheader3' width='85%' id='available' style='color:#0C0;'>"; 
			$members = array();
	...
	if($conf['usesubs']){
		...
		$text .= "<tr ".(($subs>0 && $players>0) ? "" : "style='display:none;'")." id='trsubs'>
				...
				<td align='left' class='forumheader3' width='85%' id='subs' style='color:#FF0;'>"; 
				if($players>0){
					$members = array();
					while ($row = $sql->db_Fetch()) {
						$members[] = (intval($row['member']) > 0 ? cw_getuser_name($row['member']):$row['member']);
	...
	$text .= "<tr ".(($notavailable>0) ? "" : "style='display:none;'")." id='trunavailable'>
			...
			<td align='left' class='forumheader3' width='85%' id='unavailable' style='color:#F00;'>";
			$members = array();
			while ($row = $sql->db_Fetch()) {
				$members[] = (intval($row['member']) > 0 ? cw_getuser_name($row['member']):$row['member']);
			}
