...
if (!defined('CM_ADMIN')) {
	...
	function DelMember(memberid, member){
		...
		if(sure){
			top.cmupdate.document.location = "admin.php?delmember&memberid="+memberid;
	...
	$sql->db_Select_gen("SELECT i.*, u.user_name from #clan_members_info i, #user u WHERE u.user_id=i.userid order by u.user_name");
		while($row = $sql->db_Fetch()){
			...
			$text .= "<tr id='member$memberid'>
					...
					$text .= "<td class='forumheader3' width='10' nowrap><input type='button' class='button' value='"._EDIT."' onclick=\"window.location='admin.php?editmember&memberid=$memberid'\">&nbsp;<input type='button' class='button' value='"._DEL."' onclick=\"DelMember('$memberid','$member');\"></td>
					</tr>";
			}
			$text .="</table>";
