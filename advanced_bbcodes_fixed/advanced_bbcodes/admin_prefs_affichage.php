...
for ($i=0; $i<count($prefcapt); $i++) {
        ...
        $name = $prefname[$i];
        $attributes = array('type'=>$preftype[$i], 'data' => 'int','help'=> WMLAN_06, 'writeParms'=>array('optArray'=>array(0=> LAN_DISABLED, 1=> LAN_ENABLED)));
        $text.= $rs->renderElement($prefname[$i], $pref[$name], $attributes);
        $text .="</td></tr>";
    };
