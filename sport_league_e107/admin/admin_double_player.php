...
|
|		$Source: ../e107_plugins/sport_league_e107/admin/admin_double_player.php $
|		$Revision: 0.84 $
|		$Date: 2010/02/04 $
|		$Author: ***RuSsE*** $
+---------------------------------------------------------------+
...
//  roster_player_id
    ...
    $fieldname2[] = "roster_jersy";
    $fieldtype2[] = "text";  // simple text box.
    $fieldvalu2[] = "";
    
    $fieldcapt2[] = "roster_imfeld";
    $fieldname2[] = "roster_imfeld";
    ...
    $fieldcapt2[] = "roster_pref1";
    $fieldname2[] = "roster_pref1";
    $fieldtype2[] = "text";  // simple text box.
    $fieldvalu2[] = "";
    
    $fieldcapt2[] = "roster_pref2";
    $fieldname2[] = "roster_pref2";
    $fieldtype2[] = "text";  // simple text box.
    $fieldvalu2[] = "";
    
    $fieldcapt2[] = "roster_pref3";
...
$text .= "<tr>
						<td style='width:100%; text-align:center' class='forumheader3' colspan='2'><div style='cursor:pointer' onclick=\"expandit('exp_toteram')\"><b> >> Spieler direkt zu eine Mannschaft zufügen << </b> </div></td></rd>
						</table>
...
$zahler=0;
$sql -> db_Select("league_players", "*", "players_id!='' ORDER BY players_name");
         while($row = $sql-> db_Fetch()){
         $payerdaten[$zahler]['players_id']=$row['players_id'];
         $payerdaten[$zahler]['players_name']=$row['players_name'];
         $payerdaten[$zahler]['players_burthday']=strftime("%d.%m.%Y",$row['players_burthday']);
         $payerdaten[$zahler]['double_flag']=false;
         $zahler++;
        }
if($zahler > 0)
 {
 $dublicate_count=0;
 for($i=0; $i< ($zahler-1); $i++)
    {
     $dublicate[$i]= $payerdaten[$i];
     $dublicate[$i]['anzahl']=0;
     for($j=($i+1); $j< $zahler; $j++)
       	{  
       	if($payerdaten[$j]['players_name']==$dublicate[$i]['players_name'] && !$payerdaten[$j]['double_flag'])
       		{
        	$dublicate[$i]['anzahl']++;
        	$payerdaten[$j]['double_flag']=true;
       		}
       	}
     }
	...
	for($i=0; $i< $zahler; $i++)
    { 
  	if($dublicate[$i]['anzahl']!=0)
  		{
  		$dublicate2[$dublicate_count]=$dublicate[$i];
  		$dublicate_count++;
  		}
 		}
 		
 for($i=0; $i< $dublicate_count; $i++)
    {$dublicate2[$i]['anzahl']=0;
 		for($j=0; $j< $zahler; $j++)
    	{
 			if($dublicate2[$i]['players_name']==$payerdaten[$j]['players_name'])
 				{
 				$dublicate3[$i][$dublicate2[$i]['anzahl']]=$payerdaten[$j];
 				$dublicate2[$i]['anzahl']++;
 				}
			}
		}
 ...
 $text .= "<tr>
 						<td>
 							<b>".$dublicate2[$i]['players_name']."</b> ist <b>".$dublicate2[$i]['anzahl']."</b> mal vorhanden! 
 						</td>
 					 </tr>
 					 <tr>
 					  <td>
...
$MY_STYLE[0]="backgroun:#ff0000";

	for($j=0; $j< $dublicate2[$i]['anzahl']; $j++)
    	{
    	$text .= "<tr>
 								 <td class='forumheader'";
 			if($j > 0){$text .= " style='background:#f00;'";}
 								 $text .= ">";
    		
			$text .=" mit IDs:<b>".$dublicate3[$i][$j]['players_id']."</b><br/>";
			$text .="";
			$ROSTERS= roster_count($dublicate3[$i][$j]['players_id']);
			
			
			 for($k=0; $k< (count($ROSTERS)); $k++)
			 	{
			 	$TEAM_DATEN= team_datax($ROSTERS[$k]['team']);
			  $text .="Roster: <a href='../profil.php?player_id=".$ROSTERS[$k]['id']."'>".$ROSTERS[$k]['id']."</a> 
			  				 bei: <a href='../roster.php?team=".$TEAM_DATEN['team_id']."'>".$TEAM_DATEN['team_name']."</a> 
			  	  		 in: <a href='../league_teams.php?".$TEAM_DATEN['saison_id'].".".$TEAM_DATEN['liga_id']."'>".$TEAM_DATEN['liga_name']."</a> 
			  	  		 (<a href='../league_teams.php?".$TEAM_DATEN['saison_id']."'>".$TEAM_DATEN['saison_name']."</a>)  ";				
			 		
			 		
			 	$daten = player_GTA($ROSTERS[$k]['id']);
			 	$text.="  (Spiele:".$daten['G']." Tore:".$daten['T']." Assis:".$daten['A'].") <br/>";
			 	}
			 $text .= "</td>
 					 </tr>";
			}
...
///////-----------------------------------------------
/////////////////////////////////////////////////////////////////
function player_GTA($playerID)
{
global $sql;
			$player_dat['G']=0;// Spiele gespielt
			$sql -> db_Select("league_anw", "*","anw_player_id='".$playerID."'");
			while($row = $sql-> db_Fetch())
   			{
   			$player_dat['G']++;
				}
			$player_dat['T']=0;// Tore geschoßen
			$sql -> db_Select("league_points", "*","points_player_id='".$playerID."' AND points_value= 1");
			while($row = $sql-> db_Fetch())
   			{
   			$player_dat['T']++;
				}
			$player_dat['A']=0;// Assis gemacht
			$sql -> db_Select("league_points", "*","points_player_id='".$playerID."' AND points_value= 2");
			while($row = $sql-> db_Fetch())
   			{
   			$player_dat['A']++;
				}
...
////////////////////////////////////////
/////////////////////////////
function roster_count($roster_idx)
{
	global $sql;
	$coun=0;
	$sql -> db_Select("league_roster", "*","roster_player_id='".$roster_idx."'");
	while($row = $sql-> db_Fetch())
 			{
 			$roster_ids[$coun]['id']=$row['roster_id'];
 			$roster_ids[$coun]['team']=$row['roster_team_id'];
 			$coun++;
			}
return $roster_ids;
}
function team_datax($TEAM_ID)
{global $sql;
   $qry1="
   ...
   LEFT JOIN ".MPREFIX."league_leagues AS ac ON ac.league_id=a.leagueteam_league_id   
   LEFT JOIN ".MPREFIX."league_saison AS ab ON ab.saison_id=ac.league_saison_id   
   WHERE a.leagueteam_id =".$TEAM_ID."
   		";
	$sql->db_Select_gen($qry1);

  $row = $sql-> db_Fetch();
  
  $AUSGABE['team_name']=$row['team_name'];
  $AUSGABE['team_id']=$row['leagueteam_id'];
  $AUSGABE['saison_name']=$row['saison_name'];
  $AUSGABE['saison_id']=$row['saison_id'];
  $AUSGABE['liga_name']=$row['league_name'];
  $AUSGABE['liga_id']=$row['league_id'];
 return  $AUSGABE;
}
/////////////////////////////////////////////////
?>