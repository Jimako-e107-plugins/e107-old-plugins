...
|
|		$Source: ../e107_plugins/sport_league_e107/league_table.php
|		$Revision: 0.84 $
|		$Date: 2010/02/04 $
|		$Author: ***RuSsE*** $
+---------------------------------------------------------------+
...
require_once(file_exists($lan_file) ? $lan_file :  e_PLUGIN."sport_league_e107/languages/".e_LANGUAGE."/league_table_lan.php");
...
$text ="";
if($pref['sport_league_tab_W']!="")
 {
 $LOGO_SIZE=" width='".$pref['sport_league_tab_W']."' ";
 $TAX=$LOGO_SIZE;
 }
elseif($pref['sport_league_tab_H']!="")
 {
$LOGO_SIZE=" height='".$pref['sport_league_tab_H']."' ";
$TAX=10;
 }
else
{$LOGO_SIZE="";}
...
if($saisoncount > 0)
	{
$TABEL_TOP="<td class='fcaption' style='text-align:right;width:2%;'>".LAN_LEAGUE_TABLE_31.".</td>";
if($pref['sport_league_tab_logo']=='0')
	{
	$TABEL_TOP .="<td class='fcaption' style='border-right:0px;width:".$TAX."px;text-align:center'><b>".LAN_LEAGUE_TABLE_01."</b></td>";
	}
$TABEL_TOP.="
				<td class='fcaption' style='width:40%;font-weight: bold;'>".LAN_LEAGUE_TABLE_02."</td>
...
for($i=0;$i < $count[$JS]; $i++)
		{
         ...
         while($row = $sql-> db_Fetch())
        	...
        	if($pref['sport_league_teamname_table']==1)
						{$TEAM[$JS][3][$i]=$row['team_name'];}
					elseif($pref['sport_league_teamname_table']==2)
			  		{$TEAM[$JS][3][$i]=$row['team_kurzname'];}
					else
						{$TEAM[$JS][3][$i]=LAN_LEAGUE_TABLE_32;}
  				  }
  		}
/////////////-------////////////////////////////
for($i=0;$i < $count[$JS]; $i++){
...
for($i=0; $i < $count[$JS]; $i++)     
		...
		$gesammt .="<tr><td class='".$zeilestyle."' style='text-align:right;width:2%;font-weight: bold;border-right:0px;border-top:0px;'>".($i+1)."</td>";
		if($pref['sport_league_tab_logo']=='0')
			{
			$gesammt .="<td class='".$zeilestyle."' style='border-right:0px;border-top:0px;text-align:center;'><img border='0' src='".e_PLUGIN."sport_league_e107/logos/".$TEAM[$JS][1][$i]."'".$LOGO_SIZE."></td>";
			}
...
$ausg_home .="<tr><td class='".$zeilestyle."' style='text-align:right;width:2%;font-weight: bold;border-right:0px;border-top:0px;'>".($i+1)."</td>";	
if($pref['sport_league_tab_logo']=='0')
	{
	$ausg_home .="<td class='".$zeilestyle."' style='border-right:0px;border-top:0px;text-align:center;'><img border='0' src='".e_PLUGIN."sport_league_e107/logos/".$TEAM[$JS][1][$i]."'".$LOGO_SIZE."></td>";
	}
...
for($i=0; $i < $count[$JS]; $i++)     
		...
		elseif($pref['sport_league_tab_zebra']=='0')	
			{$zeilestyle=$STYLES_KLASSES[$pref['sport_league_tab_style2']];}	
$ausg_gast .="<tr><td class='".$zeilestyle."' style='text-align:right;width:2%;font-weight: bold;border-right:0px;border-top:0px;'>".($i+1)."</td>";
if($pref['sport_league_tab_logo']=='0')
	{
	$ausg_gast .="<td class='".$zeilestyle."' style='border-right:0px;border-top:0px;text-align:center;'><img border='0' src='".e_PLUGIN."sport_league_e107/logos/".$TEAM[$JS][1][$i]."'".$LOGO_SIZE."></td>";
	}
