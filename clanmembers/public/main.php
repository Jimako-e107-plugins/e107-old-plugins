...
$sql->db_Select("clan_members_".$table, "*");
	while($row = $sql->db_Fetch()){
		$member = $row['userid'];
		$id = $row['id'];
		
		$sql1->db_Select("clan_members_info", "rank", "userid='$member'");
		$row2 = $sql1->db_Fetch();
		$rid = $row2['rank'];
			
		$sql1->db_query("UPDATE clan_members_".$table." SET rank='$rid' WHERE id='$id'");
	}
}
...
<link rel="StyleSheet" href="includes/jtip.css" type="text/css" />
...
<script type="text/javascript" src="includes/jtip.js"></script>
<?php if($cats > 0){?>
...
$text .= "<table width='".$conf['listwidth']."' border='0' cellpadding='0' cellspacing='0'><tr><td>";
$j=0;
$i=0;
$images = 0;
...
//, u.user_msnm, u.user_icq, u.user_yim, u.user_aim, u.points, u.user_posts
	...
	$sql1->db_Select_gen("SELECT l.userid, u.user_name, u.user_sess, u.user_image, r.rank, r.rimage, i.birthday, i.joindate, i.avatar, i.xfire, i.steam, i.active, i.tryout, i.votedate, i.realname, i.gender, i.location, i.country 
		FROM #".$linktable." l
...
if($t == 0){
	...
	switch($infoname){
		...
		break;
		case "Username":
			$comms = $sql2->db_Count("clan_members_comments", "(*)", "WHERE userid='$memberid'");
			$url = "";
			if($conf['enableprofile'] && (USER or $conf['profiletoguests'])){
				$newcontent = "<a href='clanmembers.php?Profile&memberid=$memberid' title='View Profile'>$member</a>".($comms && ADMIN && check_class($conf['commentsclass'])?" <a href='clanmembers.php?Comments&memberid=$memberid&width=375' class='jTip' id='comments$gtid-$memberid'><img src='".e_IMAGE."filemanager/default.png' border='0' alt='$comms "._NOTES."' /></a>":"");
			}else{
				$newcontent = "$member";
			}			
		break;
		...
		case "Steam ID":
			...
			$newcontent = "<a href='http://www.vacbanned.com/view/detail/id/$steam' target='_blank'>$steam</a>";			
		break;
		case "MSN":
			if($showcontact && $msn !=""){
		...
		case "Activity":			
			if($active == 1){
				$newcontent = "<font color='#008000'>"._ACTIVE."</font>";
			}else{
				$newcontent = "<font color='#FF0000'>"._INACTIVE."</font>";
			}
		break;
		case "Tryout":			
		...
		case "Last War":
			if(isset($pref['plug_installed']['clanwars'])){
				...
				$wid = $row5['wid'];
				if($opp_tag !="")
				$newcontent = "<a href='modules.php?name=Wars&op=Details&wid=$wid'>$opp_tag</a>";							
			}
			//End Wars
		break;
	}
