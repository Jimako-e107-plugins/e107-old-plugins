...
//-->
	...
	$text .= "<tr>
		...
		<tr>
			<td>"._INFOUserImage.":&nbsp;</td>
			<td><select name='userimgsrc'>
				<option value='0' ".($conf['userimgsrc'] == 0?"selected":"").">"._UPLIMAGE."</option>
				<option value='1' ".($conf['userimgsrc'] == 1?"selected":"").">"._USEPROFILEPIC."</option>
				<option value='2' ".($conf['userimgsrc'] == 2?"selected":"").">"._USEAVATAR."</option>
			</select></td>
		</tr>
		<tr>
		...
		<tr id='profile2' style='display:$showprofile;'>
			<td>"._COMMENTSCLASS.":&nbsp;</td>
			<td><select name='commentsclass'>";
			$classes = get_userclass_list();
			for($i=1;$i<=count($classes);$i++){
				$text .= "<option value='".$classes[$i]['userclass_id']."' ".($conf['commentsclass']==$classes[$i]['userclass_id']?"selected":"").">".$classes[$i]['userclass_name']."</option>";
			}
