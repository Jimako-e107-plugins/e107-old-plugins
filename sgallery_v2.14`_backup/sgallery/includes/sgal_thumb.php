...
if (@$PHPTHUMB_CONFIG['high_security_enabled']) {
	$hash_str = str_replace('&', '+', $_SERVER['QUERY_STRING']);
    if (!@$_GET['hash']) {
		...
		//e107 XHTML fix - SecretR
	} elseif ($_GET['hash'] != md5(str_replace('+hash='.$_GET['hash'], '',$hash_str).$PHPTHUMB_CONFIG['high_security_password'])) {
