...
class phpthumb {
	...
	function setSourceFilename($sourceFilename) {
		...
		if (is_null($this->config_output_format)) {
			...
			if (preg_match('#^[a-z]{3,4}$#i', $sourceFileExtension)) {
	...
	function ImageMagickVersion($returnRAW=false) {
		...
		if (is_null($versionstring)) {
			...
			if ($commandline) {
				...
				if (preg_match('#^Version: [^0-9]*([ 0-9\\.\\:Q/\-]+) ([0-9a-zA-Z ])*(http|file)\:#i', $versionstring[1], $matches)) {
				...
				$this->DebugMessage('ImageMagick convert --version says "'.$versionstring[0].'"', __FILE__, __LINE__);
			}
		}
				...
				if (is_array($getimagesize)) {
					...
					if ($this->source_width || $this->source_height) {
						if ($this->zc) {
							...
							if (!empty($this->fltr)) {
								foreach ($this->fltr as $key => $value) {
									if (preg_match('#^bord\|([0-9]+)#i', $value, $matches)) {
										$borderThickness = $matches[1];
										break;
									}
								}
