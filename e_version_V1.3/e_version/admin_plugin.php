...
if ($eversion_action == 'update')
{
    ...
    $evrsn_datetemp = explode("-", $_POST['eversion_date']);
    switch ($pref['eversion_dformat'])
    {
        case "d-m-Y":
            $_POST['eversion_date'] = mktime(0, 0, 0, $evrsn_datetemp[1], $evrsn_datetemp[0], $evrsn_datetemp[2]);
            break;
        case "m-d-Y":
            $_POST['eversion_date'] = mktime(0, 0, 0, $evrsn_datetemp[0], $evrsn_datetemp[1], $evrsn_datetemp[2]);

            break;
        case "Y-m-d":
            $_POST['eversion_date'] = mktime(0, 0, 0, $evrsn_datetemp[1], $evrsn_datetemp[2], $evrsn_datetemp[0]);
...
if ($eversion_action == 'dothings')
{
    ...
    if (!$eversion_dodel)
    {
        ...
        $eversion_text .= "
		<form id='eversionupdate' method='post' action='" . e_SELF . "'>
...
function gen_file()
{
    ...
    while ($evrsn_row = $evrsn_db->db_Fetch())
    {
        ...
        $evrsn_array[] = array("plugin" => $eversion_name, "version" => $eversion_major . "." . $eversion_minor . "." . $eversion_beta, "date" => "111", "URL" => $e107->base_path . $PLUGINS_DIRECTORY . "e_version/eversion.php?0.view." . $eversion_id);
        ...
        $xmlout .= "<URL>" . $e107->base_path . $PLUGINS_DIRECTORY . "e_version/eversion.php?0.view." . $eversion_id . "</URL>";
