...
if ($_POST['eclassf_action'] == "eclassf_app")
{
    ...
    foreach($eclassf_delarray as $eclassf_element)
    {
        ...
        {
            unlink("./images/classifieds/" . $eclassf_element);
        }
    }
...
if ($dh = opendir($dir))
{
    while (($file = readdir($dh)) !== false)
    {
        ...
        $filename=$file;
            ...
            $eclassf_found = $sql->db_Select("eclassf_ads", "*", "where eclassf_cpic='" . $file . "'", "nowhere");
            if (!$eclassf_found)
            {
                $eclassf_img++;
                $eclassf_text .= "<tr>
			...
			<input type='checkbox' class='tbox' style='border:0;' name='eclassf_del[]' id='delit' value='".$filename."' /></td>
			</tr>";
            }
        }
    }
