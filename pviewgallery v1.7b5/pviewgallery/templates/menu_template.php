...
include_once(e_PLUGIN."pviewgallery/templates/".$PView -> getPView_config("template")."/menu_usertemplate.php");
...
if (method_exists($UserMenu,getScroller)) {
	return $UserMenu->getScroller($imgArray);
}
...
$UserMenu = new UserMenu;	
if (method_exists($UserMenu,getMenuFeatureImg)) {
	return $UserMenu->getMenuFeatureImg();
}
...
// so it can used for template integration
	...
	foreach($allImages as $dataset) {
		...
		if($PView -> getPermission("image",$dataset['imageId'],"View")) {
			...
			$fimg .= "<img title='".$name."' src='".$thumb."' ".$imgdim." style='margin:5px;' />";
			$fimg .= "</a>";
			$fimg .= $spacer;
		}
	}
...
$UserMenu = new UserMenu;	
if (method_exists($UserMenu,getMenuFeatureAlbum)) {
	return $UserMenu->getMenuFeatureAlbum();
}
...
// so it can used for template integration
	...
	foreach($allAlbums as $dataset) {
		...
		if($PView -> getPermission("album",$dataset['albumId'],"View")) {
			...
			$falbum .= "<img title='".$name."' src='".$thumb."' ".$imgdim." style='margin:5px;' />";
			$falbum .= "</a>";
			$falbum .= $spacer;
		}
	}
...
$UserMenu = new UserMenu;	
if (method_exists($UserMenu,getMenuComments)) {
	return $UserMenu->getMenuComments();
}
    	...
    	foreach($commentArray as $key => $dataset) {
    		...
    		if ($PView -> getPView_config("force_imageSize")){
    			...
    			switch ($view) {
    			     case "img":
                     // image only
                        $pv_text .= "<tr><td style='padding-bottom:5px; text-align: center;'>".$pv_Link."<img src='".$thumb."' ".$imgdim." /></a><br />".LAN_MENU_32.$user['user_name'].", ".date('d.m.Y',$dataset['commentDate'])."</td></tr>";
                     break;
                     case "imgcomm":
                     // image and comment
                        $pv_text .= "<tr><td style='padding-bottom:5px; text-align: center;'>".$pv_Link."<img src='".$thumb."' ".$imgdim." /></a><br />".$user['user_name'].", ".date('d.m.Y',$dataset['commentDate']).":<br />";
...
$UserMenu = new UserMenu;	
if (method_exists($UserMenu,getMenuPOTD)) {
	return $UserMenu->getMenuPOTD();
}
//returns the full content of 'Picture of the day' menu
    ...
    if ($indexPOTD) {
    	...
    	$potd .= "<img title='".$name."' src='".$thumb."' ".$imgdim." style='margin:15px 0px;' />";
    	$potd .= "</a></center>";
...
$UserMenu = new UserMenu;	
if (method_exists($UserMenu,getMenuStats)) {
	return $UserMenu->getMenuStats();
}
//returns the full content of statistic menu
    ...
    if (count($ratingArray)) {
    	foreach ($ratingArray as $key => $dataset) {
    		...
    		$pv_text .= "<tr><td colspan='2' style='text-align:center;'>".$pv_Link."<img src='".$thumb."' ".$imgdim." /></a></td></tr>";
